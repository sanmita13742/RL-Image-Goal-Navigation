...
Session-based Output Layout
---------------------------
Each run creates a unique session directory under dataset/:

    dataset/
        <YYYYMMDD_HHMMSS>/
            rgb/            -- Front RGB camera frames (.png)
            depth/          -- LiDAR depth frames (.png)
            log.csv         -- Timestamps, Actions, Robot Pose
            metadata.json   -- Session provenance info

Image filenames are per-session (frame_000000.png, ...) and are
never overwritten because each run uses a fresh session directory.
...
def main():
    # ── Configuration ────────────────────────────────────────────────────────
    CONTROL_FREQ      = 10.0       # Hz (log and change action 10× per second)
    MAX_EPISODE_STEPS = 133000      # Collect 72000 steps (2 hours at 10 Hz)
    BETA              = 1          # Noise exponent: 1 = Pink (MINav paper)
    BUFFER_SIZE       = 8192       # FFT buffer size per noise channel
    SHOW_VIEWER       = False       # Set True to watch the robot live in 3D
    SAVE_IMAGES       = True       # Set True to save RGB/Depth images to disk

    # ── Session ID ──────────────────────────────────────────────────────────
    # Each run gets a unique timestamped directory, preventing overwrites.
    ...
    with open(csv_file, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        # Always write a fresh header — one CSV per session
        writer.writerow([
            "frame", "timestamp",
        ...
        # Initialize exploration policy with FFT pink-uniform noise (beta=1)
        ...
        try:
            while frame_idx < MAX_EPISODE_STEPS:
                ...
                # Depth (required for collision avoidance)
                ...
                current_v_lateral = cmd.v_lateral
                current_v_angular = cmd.v_angular

                # 4. Save and Log Data
                # Filenames are simple per-session counters; the session directory
                # guarantees uniqueness across runs — no session prefix on filenames.
                ...
                if frame_idx % 10 == 0:
                    print(
                        f"  [Log] Frame {frame_idx:04d}/{MAX_EPISODE_STEPS} | "
                        f"Prim: {prim.name} | "
                        f"Action: (v={current_v_linear:+.2f}, "
                        f"lat={current_v_lateral:+.2f}, "
                        f"w={current_v_angular:+.2f}) | "
                        f"Pose: (x={pos_x:+.2f}, y={pos_y:+.2f}, yaw={yaw:+.2f}) | "
                        f"MinDepth: {min_depth:.2f}m"
                    )
